import {ChangeDetectorRef, Component, ElementRef, EventEmitter, HostBinding, HostListener, Input, Output, OnDestroy, SimpleChanges, Inject} from '@angular/core';
...
export class PinchZoomComponent implements OnDestroy{
    ...
    @Input('limit-zoom') limitZoom: number = 0;
    @Input('disabled') disabled: boolean = false;
    @Input() disablePan: boolean = false;
    @Input() overflow: "hidden" | "visible" | undefined;
    @Input() zoomControlScale: number = 1;
    @Input() zoomControl: "one-button" | "two-buttons" | undefined;
    @Input() disableZoomControl: "disable" | "never" | "auto" | undefined;
    @Input() zoomControlPosition: "right" | "right-bottom" | "bottom" | undefined;
    @Input() backgroundColor: string = "rgba(0,0,0,0.85)";
    @Input() limitPan: boolean = false;
    ...
    public zoomOut() {
        ...
        let newScale = this.scale - Number(this.properties['zoomControlScale']);
