...
export class ChangePasswordComponent implements OnInit {
  ...
  validate() {
    ...
    if (this.changePassword.value.oldPassword.length <= 6) {
      ...
      if (this.changePassword.value.oldPassword.length == 0)
        ...
        this.oldPasswordError = "Entered character length should be greater than 6";
    }
    else if (!(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^\da-zA-Z]).{8,15}$/.test(this.changePassword.value.oldPassword))) {
      this.showOldPasswordMessage = true;
      this.oldPasswordError = "Password must have atleast a number,an uppercase, a lowercase and a special character";
    }
    else {
    ...
    if (this.changePassword.value.confirmPassword.length <= 6) {
